...
- (void)calculateXYUnits
{
    ...
    self.graphHeight = (self.frame.size.height - 2*GRAPH_MARGIN);
...
- (void)drawGraphLines
{
    ...
    [self segmentAndDrawVerticalLines];
    [self segmentAndDrawHorizontalLines];
...
- (void)drawHorizontalLineForValue:(NSNumber *)value
{
    ...
    if (self.showGrid) {
        ...
        valueLabel.font = [UIFont systemFontOfSize:(int)self.frame.size.height/20<10?(int)self.frame.size.height/20:10];
...
- (void)drawVerticalLineForDate:(NSDate *)date
{
    if (self.showGrid) {
        ...
        UILabel *dateLabel = [[UILabel alloc]initWithFrame:CGRectMake(xPointDouble + GRAPH_MARGIN - self.frame.size.width/6/2, self.frame.size.height - GRAPH_MARGIN, self.frame.size.width/6, GRAPH_MARGIN)];
        ...
        dateLabel.text = [self.dateFormatterForTimelineSegmentLines stringFromDate:date];
...
- (void)segmentAndDrawHorizontalLines
{
#define NUMBER_OF_HORIZONTAL_LINES 4
    
    int segmentRange = self.valuesRange / NUMBER_OF_HORIZONTAL_LINES;
    
    int firstValueToDisplayLineFor = self.valuesMin + segmentRange;
    
    for (int i=0; i<NUMBER_OF_HORIZONTAL_LINES; i++) {
        [self drawHorizontalLineForValue:[NSNumber numberWithInt:firstValueToDisplayLineFor + i*segmentRange]];
    }
}

- (void)segmentAndDrawVerticalLines
{
#define NUMBER_OF_VERTICAL_LINES 4
    double timelineUnitInSecondsForVerticalLines = self.timelineIntervalInSeconds/NUMBER_OF_VERTICAL_LINES;
    if (timelineUnitInSecondsForVerticalLines < 3600*24) {
        self.dateFormatterForTimelineSegmentLines = nil;
        self.dateFormatterForTimelineSegmentLines = [[NSDateFormatter alloc]init];
        self.dateFormatterForTimelineSegmentLines.dateFormat = @"dd.MM.yyyy\nHH:mm";
        for (int i=0; i<NUMBER_OF_VERTICAL_LINES; i++) {
            if (i != 0) {
                NSDate *dateToDrawLineFor = [self.fromDate dateByAddingTimeInterval:timelineUnitInSecondsForVerticalLines * i];
                [self drawVerticalLineForDate:dateToDrawLineFor];
            }
        }
    } else if (timelineUnitInSecondsForVerticalLines < 3600*24*30) {
        self.dateFormatterForTimelineSegmentLines = nil;
        self.dateFormatterForTimelineSegmentLines = [[NSDateFormatter alloc]init];
        self.dateFormatterForTimelineSegmentLines.dateFormat = @"dd.MM.yyyy";
        for (int i=0; i<NUMBER_OF_VERTICAL_LINES; i++) {
        ...
        self.dateFormatterForTimelineSegmentLines = nil;
        self.dateFormatterForTimelineSegmentLines = [[NSDateFormatter alloc]init];
        self.dateFormatterForTimelineSegmentLines.dateFormat = @"MMM yyyy";
        for (int i=0; i<NUMBER_OF_VERTICAL_LINES; i++) {
            if (i !=0) {
                NSDate *dateToDrawLineFor = [self.fromDate dateByAddingTimeInterval:timelineUnitInSecondsForVerticalLines * i];
                [self drawVerticalLineForDate:dateToDrawLineFor];
